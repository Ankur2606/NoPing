...
const { addVoiceBriefingCommands } = require('./telegramVoiceBriefing');
...
bot.onText(/\/link (.+)/, async(msg, match) => {
    try {
        ...
        await userDoc.ref.update({
            ...
            'profile.notificationPreferences.voiceBriefings': true, // Enable voice briefings by default
        ...
        bot.sendMessage(chatId, `Your Telegram account has been successfully linked with FlowSync! You can now use /briefing to get voice summaries of your important emails.`);
    } catch (error) {
...
bot.onText(/\/help/, (msg) => {
    const helpText =
        ...
        "/briefing - Get a voice summary of your important emails\n" +
...
// Add voice briefing commands to bot
try {
    addVoiceBriefingCommands(bot);

    // Schedule daily briefings (morning at 8 AM)
    if (process.env.ENABLE_SCHEDULED_BRIEFINGS === 'true') {
        const { scheduleDailyBriefings } = require('./telegramVoiceBriefing');
        scheduleDailyBriefings(bot, '0 8 * * *');
    }
} catch (error) {
    console.error('Failed to initialize voice briefing commands:', error);
}
...
console.log('Try sending /start, /link, /briefing, /test, or /help to your bot');